...
** List's syntactic sugar
   ...
   - When we talk about lists, we often talk about them in terms of “cons cells” and spines.
     ...
     above may make it more clear. The cons cells are the list datatype’s second data constructor,
     a : [a], the result of recursively prepending a value to “more list.” The cons cell is
...
** List comprehensions
   ...
   - Adding predicates
     - List comprehensions can optionally take predicates that limit the elements drawn from
       ...
       condition-placing function types we’ve looked at (for example, guards). Then the
